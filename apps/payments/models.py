...
class Payment(models.Model):
    ...
    def calculate_net_amount(self):
        ...
        if self.payment_method == 'campay':
            # CamPay: 1.5% fee, min 25 XAF, max 1000 XAF
            fee = max(Decimal("25"), min(Decimal("1000"), self.amount * Decimal("0.015")))
        elif self.payment_method == 'mtn_momo':
            # MTN Mobile Money: 1% fee, min 100 XAF, max 500 XAF
            fee = max(Decimal("100"), min(Decimal("500"), self.amount * Decimal("0.01")))
        elif self.payment_method == 'orange_money':
            # Orange Money: 0.5% fee, min 50 XAF, max 300 XAF
            fee = max(Decimal("50"), min(Decimal("300"), self.amount * Decimal("0.005")))
        else:
            fee = Decimal("0")
...
class PaymentProvider(models.Model):
    ...
    def calculate_fee(self, amount):
        ...
        fee_percentage = self.fee_percentage / Decimal("100")   # always Decimal
        fee = amount * fee_percentage

        # Use Decimal for bounds too
        fee = max(self.minimum_fee, min(self.maximum_fee, fee))
        return fee
